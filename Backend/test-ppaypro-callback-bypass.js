...
async function sendTestCallback(callbackData, callbackType = 'deposit') {
    try {
        ...
        // Use the direct route without IP whitelist (for testing only)
        const endpoint = callbackType === 'withdrawal' 
            ? 'http://localhost:3000/api/payments/ppaypro/payout-callback'
            : 'http://localhost:3000/api/payments/ppaypro/payin-callback';
        
        // Send callback
        const response = await axios.post(endpoint, callbackData, {
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded',
                'X-Forwarded-For': '127.0.0.1', // Bypass IP check
                'X-Real-IP': '127.0.0.1'
            },
...
// Alternative: Test with environment variable to bypass IP check
async function sendTestCallbackWithBypass(callbackData, callbackType = 'deposit') {
    try {
        // Generate signature
        const signature = generatePpayProSignature(callbackData);
        callbackData.sign = signature;
        
        console.log(`\n📤 Sending ${callbackType} callback (with IP bypass):`);
        console.log('Payload:', JSON.stringify(callbackData, null, 2));
        console.log('Generated Signature:', signature);
        ...
        // Send callback with headers that might bypass IP check
        const response = await axios.post(endpoint, callbackData, {
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded',
                'X-Forwarded-For': '127.0.0.1, 51.21.47.178', // Include whitelisted IP
                'X-Real-IP': '51.21.47.178',
                'User-Agent': 'PPayPro-Callback/1.0'
            },
            transformRequest: [function (data) {
                return Object.keys(data)
                    .map(key => encodeURIComponent(key) + '=' + encodeURIComponent(data[key]))
...
async function runTests() {
    console.log('🚀 Starting PPayPro Callback Tests (with IP bypass)...\n');
    ...
    for (const test of testCallbacks) {
        ...
        await sendTestCallbackWithBypass(test.data, callbackType);
