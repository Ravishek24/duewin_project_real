...
async function addUsdtwgPayGateway() {
  let sequelize = null;
  ...
  try {
    ...
    // Check if USDTWGPAY gateway already exists
    const existingGateway = await PaymentGateway.findOne({
      where: { code: 'USDTWGPAY' }
    });

    if (existingGateway) {
      console.log('✅ USDTWGPAY gateway already exists');
      ...
      console.log('✅ USDTWGPAY gateway updated and activated');
    } else {
      console.log('📝 Creating USDTWGPAY gateway...');
      
      const newGateway = await PaymentGateway.create({
        name: 'USDT WG Pay',
        code: 'USDTWGPAY',
        description: 'USDT WG Pay payment gateway for cryptocurrency deposits and withdrawals',
        logo_url: '/assets/images/payment/usdtwgpay.png',
        is_active: true,
        supports_deposit: true,
        supports_withdrawal: true,
        min_deposit: 100.00,
      ...
      console.log('✅ USDTWGPAY gateway created successfully');
