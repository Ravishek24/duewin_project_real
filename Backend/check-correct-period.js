...
async function checkCorrectPeriod() {
    ...
    try {
        ...
        const gameType = 'fiveD';  // Correct game type
        const duration = '1m';      // Correct duration
        ...
        // Also check the other keys we found
        console.log('\n=== ADDITIONAL KEYS ===');
        const totalKey = `${gameType}:${duration}:${timeline}:${periodId}:total`;
        const grossKey = `${gameType}:${duration}:${timeline}:${periodId}:gross`;
        const feesKey = `${gameType}:${duration}:${timeline}:${periodId}:fees`;
        
        const totalData = await client.get(totalKey);
        const grossData = await client.get(grossKey);
        const feesData = await client.get(feesKey);
        
        console.log('Total:', totalData);
        console.log('Gross:', grossData);
...
checkCorrectPeriod().catch(console.error); 